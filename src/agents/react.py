...
from src.agents.bg_tasks import SafetyWrapper, LlamaSafety
from src.core.llm import get_llm
from src.agents.state import StateManager
from src.core.settings import settings

class ReactAgent:
    def __init__(self, safety_checker: Optional[LlamaSafety] = None):
        self.state_manager = StateManager()
        self.tools = self._get_tools()
        self.agent = self._build_agent()
        self.checkpointer = MemorySaver()
        
        # Wrap with safety if provided
        if safety_checker:
            self.agent = SafetyWrapper(self.agent, safety_checker)
...
react_agent = ReactAgent(safety_checker=safety)